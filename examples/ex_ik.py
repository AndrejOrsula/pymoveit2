#!/usr/bin/env python3
"""
Compute inverse kinematics: end effector pose in, joint positions out.
- ros2 run pymoveit2 ex_ik.py
- ros2 run pymoveit2 ex_ik.py --ros-args -p position:="[0.3, 0.0, 0.3]" -p quat_xyzw:="[0.0, 0.0, 0.0, 1.0]"
- ros2 run pymoveit2 ex_ik.py --ros-args -p synchronous:=False
"""
...
def main() -> int:
    ...
    try:
        ...
        timeout_sec = max(0.0, float(node.get_parameter("timeout_sec").value))
        ...
        if synchronous:
            ...
            if future is None:
                ...
                node.get_logger().error("Timed out while waiting for the IK result")
                retval = None
            else:
                try:
                    ...
                    node.get_logger().error(
                        f"Failed to process the IK result: {type(error).__name__}: {error}"
                    )
                    retval = None
        if retval is None:
            node.get_logger().error("Failed to compute IK")
            print("Failed.")
        else:
        ...
        if node is not None:
            node.get_logger().error(
                f"IK example failed: {type(error).__name__}: {error}"
            )
        else:
            print(
                f"IK example failed: {type(error).__name__}: {error}", file=sys.stderr
            )
    finally:
        if cleanup(
            moveit2,
